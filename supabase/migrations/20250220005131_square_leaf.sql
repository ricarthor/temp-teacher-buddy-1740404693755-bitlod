-- Create RPC function to get quiz feedback data
...
BEGIN
  ...
  LATERAL jsonb_object_keys(rating_fields) key
  ...
  WITH rating_stats AS (
    SELECT 
      key as rating_type,
      AVG((rating_fields->>key)::numeric) as avg_rating,
      COUNT(*) as count
    FROM students_feedback,
    LATERAL jsonb_object_keys(rating_fields) key
    WHERE quiz_id = p_quiz_id
  ...
  WITH pairs AS (
    SELECT 
      ...
      r2.key as type2,
      CORR(
        (sf.rating_fields->>r1.key)::numeric,
        (sf.rating_fields->>r2.key)::numeric
      ) as correlation
    FROM students_feedback sf
    CROSS JOIN LATERAL jsonb_object_keys(rating_fields) r1
    CROSS JOIN LATERAL jsonb_object_keys(rating_fields) r2
    WHERE quiz_id = p_quiz_id
