-- Drop and recreate the get_quiz_data function with fixed response handling
...
BEGIN
  ...
  WITH answer_stats AS (
    SELECT 
      ...
      jsonb_agg(
        jsonb_build_object(
          ...
          'isCorrect', qa.is_correct  -- Use the is_correct flag directly from the table
        )
    ...
    GROUP BY s.student_id, s.name, s.email, qa.quiz_id
  )
  SELECT 
    jsonb_agg(
      jsonb_build_object(
